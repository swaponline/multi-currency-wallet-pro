 ...
 * Domain Path: /lang
 * License: GNU General Public License version 3.91
 * License URI: http://www.gnu.org/licenses/gpl-3.91.html
 */
...
define( 'MCWALLET_VER', '1.0.112' );
define( 'MCWALLET_BUILD_VER', '62ff98' );
...
function foo_show_extra_profile_fields($user)
{
    ...
    if (isset($data[1])) {
       // var_dump($data[1]); ?>
        <ul style="font-size: 17px"> <?php
            foreach ($data[1] as $k => $item) {

                if($k == 'WPuserUid') continue;
                ?>
                <li><b style="font-size: 18px; padding-bottom: 11px"><?php echo esc_html($k); ?></b>
                    <ul>
                        <?php
                        if(is_array($item)) {
                        foreach ($item as $j => $el) {
                            ...
                            <li style="margin-top: 10px; padding-left: 21px"><b><?php echo esc_html($j); ?>:</b>
